...
def compute_resize_scale(image_shape, tar_height=768, tar_width=1024):
    """ Compute an image scale such that the image size is constrained to min_side and max_side.
    ...
    scale = (float(tar_height)/float(rows),float(tar_width)/float(cols))

    return np.array(scale)

# def resize_image(img, min_side=800, max_side=1333):
def resize_image(img, tar_height=512, tar_width=768):
    """ Resize an image such that the size is constrained to min_side and max_side.
    ...
    """
    # compute scale to resize the image
    scale = compute_resize_scale(img.shape, tar_height=tar_height, tar_width=tar_width)

    # resize the image with the computed scale
    # img = cv2.resize(img, None, fx=scale, fy=scale)
    img = cv2.resize(img, (tar_width,tar_height)) #opencv are using (x,y)<==>(cols,rows)
